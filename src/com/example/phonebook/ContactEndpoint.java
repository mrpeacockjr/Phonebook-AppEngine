...
@Api(name = "contactendpoint", namespace = @ApiNamespace(ownerDomain = "example.com", ownerName = "example.com", packagePath = "phonebook"))
public class ContactEndpoint {
	...
	public CollectionResponse<Contact> listContact(
		...
		try {
			...
			Query query = mgr.createQuery("select from Contact as Contact");
			...
			execute = (List<Contact>) query.getResultList();
	...
	public Contact getContact(@Named("id") Long id) {
		...
		try {
			contact = mgr.find(Contact.class, id);
		} finally {
			mgr.close();
		}
		return contact;
	...
	private boolean containsContact(Contact contact) {
		...
		try {
			Contact item = mgr.find(Contact.class, contact.getKey());
			if (item == null) {
				contains = false;
			}
		} finally {
